...
- [x] **Webview Availability API Integration**: Connected the availability portal to dynamically load, save, and clear weekly driver shifts from the database via `GET /api/DriverApp/Availabilities`, `POST /api/DriverApp/SetAvailability`, and `GET /api/DriverApp/DeleteAvailability`. Decodes the active user ID from JWT tokens and maps slots (AM as `08:00-16:00`, PM as `16:00-23:59`, and Both as `08:00-23:59`) correctly.

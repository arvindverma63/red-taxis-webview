...
- [x] **Webview Availability API Integration**: Connected the availability portal to dynamically load, save, and clear weekly driver shifts from the database via `GET /api/DriverApp/Availabilities`, `POST /api/DriverApp/SetAvailability`, and `GET /api/DriverApp/DeleteAvailability`. Refactored the UI with a premium light-themed card layout matching other screens, and implemented calendar week navigation controls (including a calendar date picker pop-up) to support flexible scheduling. Solved the native Android WebView NullPointerException crash by replacing HTML5 time input pickers with custom styled hour/minute select dropdowns. Implemented timezone-safe local date formatting and parsing, and aligned select dropdown hour/minute value options with robust leading-zero string normalization helpers to prevent single-digit time serialization values from resetting time pickers to 00:00 on save.
